...
import { chooseDefinedValue } from '../helpers/parse';
...
export default class VueMappInput extends InputElement {
  ...
  private processMask(value) {
    ...
    for (let i = 0, x = 1; x && i < mask.length; ++i) {
      ...
      const c = value.charAt(i - cOffset);
      const m = mask.charAt(i);
  ...
  public drawBoxSvg() {
    ...
    const path = 'M' + (expanded ? startX + labelWidth : startX) + ',' + 1
...
<template>
  ...
  >
    <div
      ref="container"
      class="vm-input__container"
    >
      <div
        ref="label"
        v-if="$_label"
        :class="{ 'is--noanimate': noanimate }"
        class="vm-input__label"
      >
        {{ $_label }}
        <span v-if="$_required">*</span>
      </div>

      <div
        v-if="$slots.left"
        class="vm-input__left"
      >
        <slot name="left" />
      </div>
      ...
      <svg v-if="$_box">
        <path
          ref="svgboxpath"
          class="vm-input__outline"
        />
      </svg>
      <div
        v-if="!$_box"
        class="vm-input__divider"
      >
        <div
          v-if="loading || validateStatus === 'process'"
          class="vm-input__divider-loader"
        />
      </div>
    </div>
    <div
